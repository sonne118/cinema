...
namespace ApiApplication.Database
{
    public class SampleData
    {
        ...
        private static List<SeatEntity1> GenerateSeats(int auditoriumId, short rows, short seatsPerRow)
        {
            var seats = new List<SeatEntity1>();
            for (short r = 1; r <= rows; r++)
                for (short s = 1; s <= seatsPerRow; s++)
                    seats.Add(new SeatEntity1 { AuditoriumId = auditoriumId, Row = r, SeatNumber = s });
